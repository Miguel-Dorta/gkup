...
func (r *Repo) Create(hashAlgorithm string) error {
	pkg.Log.Infof("Creating directory in %s", r.path)
	pkg.Log.Debug("Checking if exists something in the repo path")
	...
	if err != nil {
		...
		pkg.Log.Debug("Creating repo folder")
